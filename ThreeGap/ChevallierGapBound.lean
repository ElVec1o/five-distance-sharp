...
open Classical in
/-- **The best-approximation denominators** `qₙ`, with the positivity proof carried alongside (so the
...
theorem bestDenom_int_strictMono (hr : RecordsContinue r) :
  ...
  intro a b h
  show (bestDenom r hr a : ℤ) < (bestDenom r hr b : ℤ)
...
theorem bestDenom_bracket (hr : RecordsContinue r) {N : ℕ} (hN : 1 ≤ N) :
  ...
  strictMono_bracket _ (bestDenom_strictMono r hr) (by rw [show bestDenom r hr 0 = 1 from rfl]; omega)
...
theorem bestDenom_bracket2 (hr : RecordsContinue r) {N : ℕ} (hN : 2 ≤ N) :
  ...
  strictMono_bracket (fun k => 2 * bestDenom r hr k)
    (fun a b h => by
      show 2 * bestDenom r hr a < 2 * bestDenom r hr b
...
for the sup norm by the orthant pigeonhole (`q_{n+2^d} ≥ 2q_n`, Chevallier survey §2.4.1 / Lagarias),
...
bracketing indices `n` (`qₙ ≤ N < qₙ₊₁`) and `m` (`2qₘ ≤ N < 2qₘ₊₁`) are produced internally from the
