...
import ThreeGap.FiveDistance
import Mathlib.Geometry.Euclidean.Angle.Unoriented.TriangleInequality

/-!
# Toward the sharp Euclidean five-distance theorem via Haynes–Marklof Theorem 8

The repo proves `g₂ ≤ 6` (`gap_count_euclidean`) and reduces the sharp `g₂ ≤ 5` to the planar packing
...
the angular separation. HM defeat the sixth vector using a **height coordinate** `u = t/N` in addition
...
* `recVec_neg` / `norm_recVec_neg` — **antipodal record symmetry** (atom HM-1): the `(−t, −p)` record
...
of `vⱼ, vₖ` (`i < j < k`). The final count: 5 vectors pairwise `> π/3` are cyclically ordered, so the
...
worst value `< −0.015`); the constraint set is **minimal** (dropping either angle constraint yields a
counterexample). The remaining task is the **SOS/Positivstellensatz certificate**: it is not a linear
...
particular, if both `∠(vⱼ,vᵢ) > π/3` and `∠(vᵢ,vₖ) > π/3` then `∠(vⱼ,vₖ) > 2π/3` — the angle bound the
...
/-- **Cone forces a wide opening (atom HM-7d, corollary).** If `vᵢ` is in the closed positive cone of
`vⱼ, vₖ` and is more than `π/3` from each, then `∠(vⱼ,vₖ) > 2π/3`. This is the obtuse-angle bound the
...
`‖v₀ − vⱼ − vₖ‖ < ‖vₖ‖` — contradicting the record inequality (5.3). **Proof** (clean trig, replacing
...
`< 0` at `r=0` (obtuse cone) and `≤ 0` at `r=b` (`cone_trig_crux`), hence `< 0` on `r ∈ (0,b)` by the
