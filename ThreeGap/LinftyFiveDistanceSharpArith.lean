...
This file shows the bound is **sharp** by an explicit, dynamics-free witness, mirroring the Euclidean
...
with record minima at `d = 1, 3, 9, 10, 12, 13` (values `15 > 12 > 11 > 9 > 8 > 7`). The four records
inside the doubling window `(⌈13/2⌉, 13] = (7, 13]` are `9, 10, 12, 13` — giving the **five** distinct
...
comparison is between *integers*, and the rational→irrational transport gap is *linear* (no `√`-gap):
...
/-- **`card ≥ 5` from the four record drops.** If the defect drops strictly below the whole preceding
...
Kronecker vector `α` with an irrational coordinate and an `N ≥ 2` for which the orbit `{0, α, …, Nα}`
