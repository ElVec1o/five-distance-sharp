...
following N. Chevallier, *Distances dans la suite des multiples d'un point du tore à deux dimensions*,
...
`{x_0, …, x_N}` is `D_q = min_{1 ≤ j ≤ max(q, N−q)} d(x_0, x_j)`, because (isometry) the distances from
...
import ThreeGap.ChevallierGapBound
...
`r(q_k) < r j` for `1 ≤ j < q_k`. (This is the full best-approximation property: `q_k` strictly beats
...
theorem chevallier_gap_count_le (hr : RecordsContinue r) (K : ℕ)
  ...
  have hnm : n ≤ m + K :=
    index_bound_doubling (fun k => (bestDenom r hr k : ℤ)) (bestDenom_int_strictMono r hr) K
      (fun k => by dsimp only; exact_mod_cast hgrowth k) (N := (N : ℤ)) (m := m) (n := n)
      (by dsimp only; exact_mod_cast hn1) (by dsimp only; exact_mod_cast hm2)
  omega
