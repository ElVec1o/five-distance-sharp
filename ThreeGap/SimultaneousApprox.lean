...
This is `delta_diff_le`. It is the algebraic input that makes the remainder vectors *spread out*; the
...
below `q (k+1)` has defect `≥ δ_{q k}`). In a *doubling window* `q (k+K) < q (k+1) + q k`, any two of
