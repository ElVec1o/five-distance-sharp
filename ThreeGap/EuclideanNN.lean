...
nearest-neighbour distance, completing the Euclidean five-distance bound `g₂ ≤ 6` as a statement about
the actual orbit `{0, α, …, Nα}` on `𝕋²`.

The isometry reduction (`TorusReduction.gapVal_eq_nnDist`) used only the **symmetry** of the cost, so
...
import ThreeGap.EuclideanRecords
...
/-- **`g₂ ≤ 6` for the actual Euclidean nearest-neighbour distances on `𝕋²` (unconditional).** For any
`α : Fin 2 → ℝ` with an irrational coordinate and `N ≥ 2`, the orbit `{0, α, …, Nα}` has at most **six**
