...
This is the core of the Euclidean route to `g₂ ≤ 6`: the best-simultaneous-approximation denominators
in the **Euclidean (`L²`) norm** on `𝕋²` at least double every **5** steps. Combined with Chevallier's
...
best-approximation separation: their index differences lie in `(0, qₙ)`, so `hbest` forces the defect
up). The metric→angle crux (`EuclideanAngle`) turns that into pairwise angles **strictly** `> π/3`
(strict because the defects strictly decrease, so all but the first vector is shorter than `δₙ`), and
the planar packing count (`EuclideanPacking.not_six_separated`) says at most **5** such vectors fit —
contradiction.

Stated, like `EuclideanGrowth.euclidean_growth_additive`, over an abstract best-approximation sequence
...
import ThreeGap.EuclideanPacking
import ThreeGap.EuclideanGrowth
