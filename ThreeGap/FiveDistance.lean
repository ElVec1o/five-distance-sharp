...
-/
import ThreeGap.EuclideanAngle
...
This composes with the already-proven `EuclideanAngle.norm_sub_lt_max_of_angle_lt` (their Prop 2) and
...
‖v₀‖²·⟪vᵢ,vⱼ⟫`. Writing `(aᵢ,dᵢ) = (⟪v₀,vᵢ⟫, det2 v₀ vᵢ)/(‖v₀‖‖vᵢ‖)` as the cosine/sine of the signed
angle of `vᵢ` from `v₀`, this says `cos∠(vᵢ,vⱼ) = aᵢaⱼ + dᵢdⱼ` — the full planar trigonometry, derived
...
⟪v₀,vⱼ⟫ · det2 v₀ vᵢ`. In the `(aᵢ,dᵢ) = (cos,sin)` reading, `sin∠(vᵢ→vⱼ) = dⱼaᵢ − aⱼdᵢ` — so the sign
...
`≤ π`. This is the real-number heart of "4 vectors in a half-plane can't be pairwise `> π/3`" (the arc
has length `≤ π`). -/
...
is `≤ 2π/3`. The `2π/3`-length analogue of `not_four_in_interval` — it bounds the number of vectors on
...
apart are impossible: their reference angles `βᵢ = ∠(v₀, w i)` lie in the length-`π` interval `[0,π]`
...
open RealInnerProductSpace in
/-- **At most 2 vectors strictly on one side of `v₀` and `> π/3` from `v₀`, pairwise `> π/3`.** Three
...
contradicting `not_three_in_interval`. This is the per-side ceiling the Theorem-8 cyclic argument uses
...
open RealInnerProductSpace in
/-- **At most one vector can be antiparallel to `v₀`.** If `det2 v₀ wᵢ = 0` and `det2 v₀ wⱼ = 0` with
...
theorem antiparallel_unique (v0 wi wj : EuclideanSpace ℝ (Fin 2)) (h0 : v0 ≠ 0) (hwi : wi ≠ 0)
    ...
    (hbi : π / 3 < InnerProductGeometry.angle v0 wi) (hbj : π / 3 < InnerProductGeometry.angle v0 wj)
...
theorem exists_pos_det2 (v0 : EuclideanSpace ℝ (Fin 2)) (w : Fin 4 → EuclideanSpace ℝ (Fin 2))
  ...
  have hzero : (Finset.univ.filter (fun i => det2 v0 (w i) = 0)).card ≤ 1 := by
    ...
    by_contra hab
    exact antiparallel_unique v0 (w a) (w b) h0 (hw a) (hw b) ha hb (hbeta a) (hbeta b) (hsep a b hab)
  ...
  have hcongr : (Finset.univ.filter (fun i => ¬ det2 v0 (w i) < 0))
    ...
    Finset.filter_congr (fun i _ => by have := hcon i; rw [not_lt]; exact ⟨fun h => le_antisymm this h,
      fun h => h.ge⟩)
...
theorem angle_antiparallel {v0 w : EuclideanSpace ℝ (Fin 2)} (u : EuclideanSpace ℝ (Fin 2))
  ...
  rw [hc, InnerProductGeometry.angle_smul_left_of_neg _ _ hcneg, InnerProductGeometry.angle_neg_left]
...
fact (from `det2_neg_of_opp_side_of_sum_lt_pi` once the gap argument gives `βⱼ+βₖ < π`) is the single
...
`∠(v₀,w) > π/3` (ruling out codirectional), then `∠(v₀,w) = π`. Extracted from `antiparallel_unique`'s
...
sum is `< π` we are done. Otherwise, place all 4 non-`v 0` vectors at signed angles `ψ` (positive side
...
theorem exists_flanking_pair {v : Fin 5 → EuclideanSpace ℝ (Fin 2)} (hne : ∀ i, v i ≠ 0)
  ...
  rcases lt_or_ge (InnerProductGeometry.angle (v 0) (w js) + InnerProductGeometry.angle (v 0) (w ks)) π
    with hlt | hge
...
the cone property for `v 0` (the shortest vector lies in no open cone of two higher-indexed vectors).
