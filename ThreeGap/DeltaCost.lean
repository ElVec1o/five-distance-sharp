...
* **The record structure of `bestDenom`** supplies `hdec` (`bestDenom_cost_antitone`) and the full
  best-approximation property `hbsad` (`bestDenom_strict_floor`: `q_k` strictly beats *every* smaller
...
`g_∞ ≤ 2^d + 1` holds for the sup-norm defect cost given only `RecordsContinue` — the growth geometry
...
import ThreeGap.ChevallierCount
import ThreeGap.SupNormGrowth
...
/-- `hbsad` for the record denominators: each `q_k` is a **strict** record minimum over *all* smaller
...
*only* hypothesis is `RecordsContinue` (the best approximations of `α` improve without bound, i.e. the
appropriate irrationality of `α`). The remaining step to the geometric three-distance statement is the
