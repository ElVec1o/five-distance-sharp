...
and the **strict** form `angle u v > π/3` when one of the vectors is strictly shorter than `δ` (which
...
* The angular **gap-sum** (`6 × (>60°) > 360°`) with the *strict* bound gives `≤ 5` (`K = 5`,
  `g₂ ≤ 6`).
* The **sharp** `≤ 4` (`K = 4`, `g₂ ≤ 5`) is **Romanov's theorem** (M.V. Romanov, *Moscow Univ. Math.
  ...
  hexagon separation lemma — a strictly finer (and figure-dependent) input than the angle bound here.
...
/-- **Haynes–Marklof Proposition 2** (the cone-partition route to the *sharp* Euclidean five-distance
...
Romanov's growth lemma and the angular↔magnitude coupling: within any `< π/3` cone of directions, two
...
/-- **The record-angle bound** (the contrapositive form Haynes–Marklof Theorem 8 consumes). If `u` is
